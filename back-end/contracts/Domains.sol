...
contract Domains is ERC721URIStorage {
    ...
    function register(string calldata name) public payable {
        ...
        string memory userDomain = string(abi.encodePacked(name, ".", topLevelDomain));

        // Create the SVG (image) for the NFT with the name
        string memory finalSvg = string(abi.encodePacked(svgPartOne, userDomain, svgPartTwo));

        uint256 newRecordId = _tokenIds.current();
        uint256 length = StringUtils.strlen(name);
        string memory strLen = Strings.toString(length);

        console.log("Registering %s on the contract with tokenID %d", userDomain, newRecordId);
